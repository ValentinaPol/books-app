const getData = () => {
    fetch('https://books-app530.herokuapp.com/api/books').then(
        res => {
            return res.json();
        }
    ).then(
...
const postData = (e) => {
    ...
    fetch('https://books-app530.herokuapp.com/api/books', options).then(
        res => {
            return res.json();
        }
    ).then(
...
const deleteData = (e) => {
    if(e.target.classList.contains('delete-btn')){
        ...
        fetch(`https://books-app530.herokuapp.com/api/books/${bookId}`, options).then(
            res => {
                return res.json();
            }
        ).then(
...
const editData = (e) => {
    ...
    fetch(`https://books-app530.herokuapp.com/api/books/${currentEditBook}`, options).then(
        res => {
            return res.json();
        }
    ).then(
